...
{
    class SqlServer<T> : BaseSql<AliGoods>
        where T : class
    {
        ...
        {
            AliGoods localali = x;
            string query = $"Insert into aliGoods(@id, @Price, @Name) values ({localali.Id},{localali.Price},{localali.Name})";
        ...
        public void AddRange(AliGoods[] arr)
        {
            ...
            {
                string query = $"Insert into aliGoods(@id, @Price, @Name) values ({localali[i].Id},{localali[i].Price},{localali[i].Name})";
                using (SqlConnection connection = new SqlConnection(connectionstring))
                {
                    connection.Open();
